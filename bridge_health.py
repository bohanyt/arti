...
def probe_openrouter_api(config: dict, timeout: int = 15) -> tuple[str, str]:
    ...
    model = config.get("openrouter_live_model", "nvidia/nemotron-3-super-120b-a12b:free")
    try:
        res = requests.post(
...
def probe_observer_providers(config: dict) -> list[CheckRow]:
    ...
    cursor_role WAJIB ikut: sesudah split model 2026-08-03 observer punya
    sesi/model sendiri (grok-4.5/high) — tanpa ini halaman health memprobe
    chain observer memakai model role scout (composer) dan hasilnya bohong.
    """
    return probe_scouter_providers({
        **config,
        "scouter_provider_chain": config.get("observer_provider_chain"),
