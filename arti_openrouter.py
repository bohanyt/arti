...
def _openrouter_chat_stream(
    ...
    *,
    ...
    # JANGAN pakai iter_lines(decode_unicode=True): tanpa charset eksplisit di header,
    # requests mendekode body pakai ISO-8859-1 (default HTTP), bukan UTF-8. Untuk SSE
    # `text/event-stream` itu selalu terjadi, dan hasilnya teks Indonesia dengan karakter
    # non-ASCII jadi mojibake — sumber "2026â€"06â€"07" di vault/sessions/*_reflection.md.
    # Rusaknya permanen karena reflection langsung ditulis ke vault.
    # Dekode byte sendiri sebagai UTF-8, sama seperti arti_groq_stream.iter_sse_json_lines.
    for raw_bytes in res.iter_lines():
        if not raw_bytes:
            continue
        raw = raw_bytes.decode("utf-8", errors="replace") if isinstance(raw_bytes, bytes) else raw_bytes
        line = raw.strip()
...
def openrouter_live_model_chain(config: dict) -> list[str]:
    """Chain live OpenRouter — default: Nemotron Super 120B lalu Gemma 4.

    Default lama (`poolside/laguna-xs.2:free` + `owl-alpha`) sudah 404 sejak
...
def run_post_stream_reflection(
    ...
    chain = [
        ...
        config.get("openrouter_reflection_fallback_model", "google/gemma-4-26b-a4b-it:free"),
        config.get("openrouter_reflection_last_resort", "poolside/laguna-xs-2.1:free"),
    ]
...
Tulis analisis singkat bahasa Indonesia (maksimal 3 paragraf, tanpa mengulang isi
transcript baris per baris), lalu JSON terpisah di akhir.

Standar isi JSON — sesi yang sepi WAJAR menghasilkan list kosong. Lebih baik kosong
daripada diisi hal yang tidak berguna:

learnings — fakta atau pola yang masih berguna BULAN DEPAN.
  BOLEH  : cerita pribadi streamer, preferensi tetap, rencana, keputusan yang diambil,
           pola perilaku penonton yang berulang.
  DILARANG: menceritakan ulang apa yang terjadi di sesi ini, mendeskripsikan gaya bicara
           Arti yang sudah jelas dari persona-nya, dan mencatat bahwa sesi ini sepi.
           Kalau tidak ada interaksi, learnings = [].

viewer_updates — hanya penonton yang benar-benar muncul di transcript dengan sesuatu
  yang layak diingat. Jangan mengarang nama.

tech_debt — HANYA masalah yang buktinya ada di LOG ERROR atau terlihat jelas di
  transcript. Jangan menyarankan pemeriksaan spekulatif ("mungkin perlu cek setelan X")
  kalau tidak ada gejalanya. Kalau log bersih, tech_debt = [].

{{
  "learnings": ["fakta tahan lama; [] kalau tidak ada"],
  "viewer_updates": [{{"name": "nick", "notes": "1 baris"}}],
  "tech_debt": ["masalah dengan bukti di log; [] kalau log bersih"]
}}"""
