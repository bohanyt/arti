...
def test_stuck_rate_limited_other_failures_silent():
    ...
    # Jarak antar-narasi dibaca dari konstanta modul ([date removed] naik 120->300
    # karena "kakimu ngambek" jadi keluhan berulang di stream) — tes ini tidak
    # boleh mengunci angkanya, cuma perilakunya: dalam jendela = diam.
    from arti_minecraft import _STUCK_REACT_GAP_SEC as GAP
    assert map_event_to_reaction(stuck, lim, 100.0, CFG) is not None
    assert map_event_to_reaction(stuck, lim, 100.0 + GAP / 2, CFG) is None
    assert map_event_to_reaction(stuck, lim, 101.0 + GAP, CFG) is not None
