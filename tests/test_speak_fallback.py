...
def test_supertone_failure_falls_back_to_edge_with_identical_text(
    ...
    # Tag ekspresi dikonversi ke vokalisasi SEBELUM engine dipilih, jadi
    # "same text" = teks hasil strip yang sama untuk kedua engine.
    expected_text = bridge.strip_tts_expression_tags(text)
    assert edge.calls[0] == expected_text, (
        f"edge_tts received different text for failure {failure_label!r}: "
        f"{edge.calls[0]!r} != {expected_text!r}"
    )
...
# ---------------------------------------------------------------------------
def test_edge_tts_path_converts_tags_to_vocalizations(monkeypatch):
    """Tag ekspresi TIDAK boleh sampai ke edge_tts sebagai literal "<laugh>"
    (edge akan mengejanya). speak() mengonversinya ke vokalisasi via
    strip_tts_expression_tags sebelum engine dipilih.

    (Menggantikan Req 12.5 lama "tags verbatim" — perilaku berubah saat
    konversi tag dipindah ke awal speak().)
    """
    ...
    assert edge.count == 1
    handed = edge.calls[0]
    assert handed == bridge.strip_tts_expression_tags(text)
    for tag in ("<laugh>", "<breath>", "<sigh>"):
        assert tag not in handed, (
            f"tag mentah {tag!r} bocor ke edge_tts: {handed!r}"
        )
...
def test_supertone_success_does_not_fall_back(monkeypatch):
    ...
    assert text_arg == bridge.strip_tts_expression_tags(text)
    assert word_timings == []
