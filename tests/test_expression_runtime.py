...
# --- permintaan ekspresi EKSPLISIT menang atas tag LLM -------------------------
# Live 2026-08-02: "kalo kamu pasang muka marah?" -> LLM tag senang -> wajah
# tetap default; Bohan harus maksa dua kali. Permintaan eksplisit viewer/streamer
# ("pasang muka X") harus langsung menang.


def test_explicit_face_request_overrides_llm_tag():
    import arti_expression_runtime as er

    assert er.resolve_turn_emotion("kalo kamu pasang muka marah?", "senang") == "marah"
    assert er.resolve_turn_emotion("pasang aja muka sedih dong", "neutral") == "sedih"
    assert er.resolve_turn_emotion("coba ekspresi bingung", "senang") == "bingung"
    assert er.resolve_turn_emotion("tunjukin muka senang dong", "sedih") == "senang"


def test_non_explicit_hint_still_defers_to_llm_tag():
    import arti_expression_runtime as er

    # sekadar menyebut "sedih" tanpa minta ekspresi: tag LLM tetap menang
    assert er.resolve_turn_emotion("aku lagi sedih nih", "senang") == "senang"
    # dan tetap jadi hint saat LLM neutral (perilaku lama)
    assert er.resolve_turn_emotion("aku lagi sedih nih", "neutral") == "sedih"
