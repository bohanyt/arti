...
@given(
    text=st.text(
        # Exclude lone surrogates (kategori Cs): bukan teks UTF-8 valid —
        # assert .encode("utf-8") di bawah akan meledak di test-nya sendiri,
        # dan edge_tts tidak pernah mengirim surrogate di WordBoundary.
        alphabet=st.characters(min_codepoint=0x0080, exclude_categories=["Cs"]),
        min_size=1,
        max_size=80,
    )
)
