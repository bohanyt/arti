...
def test_property_7_supertone_broadcasts_empty_word_timings(harness, monkeypatch,
    ...
    **Validates: Requirements 13.1**
    ...
    # speak() menormalkan teks via strip_tts_expression_tags; kalau hasilnya
    # kosong, speak() return lebih awal tanpa broadcast/playback sama sekali.
    expected_text = bridge.strip_tts_expression_tags(text)

    _run_supertone(harness, engine, text)

    if not expected_text:
        assert harness.broadcast_calls == [], "teks kosong: tidak boleh broadcast"
        assert harness.play_calls == 0, "teks kosong: tidak boleh playback"
        return
    ...
    # Sejak v0.5.8 (phrase karaoke "Option C"): Supertone tidak punya timing
    # per kata, jadi bridge broadcast ESTIMASI phrase-timing proporsional
    # panjang karakter — bukan lagi list kosong (Req 13.1 versi lama).
    starts = [wt["start"] for wt in word_timings]
    for wt in word_timings:
        assert set(wt.keys()) == {"word", "start", "duration"}, (
            f"entri phrase-timing harus punya word/start/duration; got {wt!r}"
        )
        assert wt["duration"] >= 0
    assert starts == sorted(starts), "phrase timing harus monoton naik"
    assert full_text == expected_text, (
        "teks broadcast = hasil strip_tts_expression_tags (bukan input mentah)"
    )
...
def test_property_7_edge_tts_broadcasts_wordboundary_derived(harness, monkeypatch,
    ...
    **Validates: Requirements 13.2**
    ...
    # speak() menormalkan teks dulu; input yang strip jadi kosong tidak pernah
    # sampai ke edge_tts (early return, tanpa broadcast).
    expected_text = bridge.strip_tts_expression_tags(text)
