...
# default "cheerful" yang di-fallback-kan oleh `get_current_mood()`, plus
# output _emotion_to_mood() yang ditulis scouter ke ARTI_MOOD_STATE.json
# saat live (happy/sad/angry/confused/lazy — "lazy" = default netral).
VALID_MOOD_STRINGS = {
    "cheerful",
    "excited",
    "bingung",
...
def test_bug_condition_subprocess_startup_no_name_error():
    ...
    # 60s, bukan 20s: bridge asli di-spawn di sini dan startup-nya melakukan health
    # check (mic test 2 detik, koneksi VTS, probe API) sebelum `main_loop` mencetak
    # LISTENING_STATE_LOG. Sendirian butuh ~19,5 detik — nyaris menyentuh batas 20s
    # lama, dan lewat batas begitu suite penuh jalan berbarengan.
    result = _run_bridge_subprocess(timeout_seconds=60.0)
...
# Urutan v0.6.1-stable (terverifikasi live): rework observer/scouter v0.6
# memindahkan start_summarizer keluar dari main_loop (scouter start on-demand)
# dan menggeser voice_listener_worker setelah init_global_hotkey.
EXPECTED_MAIN_LOOP_STARTUP_ORDER = [
    ...
    # obs-subtitle-integration task 6.1: in-process Subtitle Server duduk
    # di antara context-load terakhir dan operasi chat-history pertama.
...
def _module_constants_snapshot(module) -> dict[str, Any]:
    ...
    for name in sorted(vars(module).keys()):
        ...
        if type_name in {
            ...
            continue
        if name == "CONFIG":
            # CONFIG di-snapshot sebagai STRUKTUR (sorted key list) saja.
            # Nilainya tergantung .env / config_local.json / live_session.json
            # (wizard bisa mengubah port dll di antara dua run) DAN berisi
            # API key asli — dua alasan untuk tidak menyimpan value-nya.
            snapshot[name] = sorted(str(k) for k in value.keys())
            continue
        # Global lain yang nilainya dari env (.env dev berisi key asli) —
        # snapshot keberadaannya saja, jangan value-nya.
        if name in {"openrouter_api_key"}:
            snapshot[name] = "<env-derived:str>"
            continue
...
@settings(
    suppress_health_check=[HealthCheck.function_scoped_fixture],
    max_examples=20,
    deadline=None,  # `_isolated_mood_dir` melakukan importlib.reload(module) per example
                    # (~1.2s), sama seperti 2a/2b di atas. Kontrak ini tentang SCHEMA
                    # file, bukan timing — deadline default 200ms bikin flaky saat suite
                    # penuh jalan (terukur 265ms di mesin yang sibuk).
)
@given(
    mood=st.text(
...
# --- wizard "simpan default" TIDAK boleh menulis ulang source (audit 3/8) -------


def test_save_config_writes_config_local_not_source(tmp_path, monkeypatch):
    """Dulu _save_config_to_file menulis ulang hermes_vtuber_bridge.py sendiri:
    (a) nilai sesi live ikut ter-commit (subtitle_port 9992 -> subtitle OBS
    mati di checkout bersih), (b) percuma karena config_local dimuat SETELAH
    CONFIG dan menang. Sekarang: tulis ke config_local.json, source utuh."""
    import json as _json

    import hermes_vtuber_bridge as b

    src_path = BRIDGE_FILE
    before = src_path.read_bytes()

    local = tmp_path / "config_local.json"
    local.write_text(
        _json.dumps({"streamer_name": "bohan", "vts_model_dir": "D:/x"}),
        encoding="utf-8",
    )
    monkeypatch.setattr(b, "_SCRIPT_DIR", str(tmp_path))
    monkeypatch.setitem(b.CONFIG, "youtube_video_id", "aBcDeFgHiJk")
    monkeypatch.setitem(b.CONFIG, "youtube_chat_enabled", True)
    monkeypatch.setitem(b.CONFIG, "subtitle_port", 9991)

    b._save_config_to_file()

    saved = _json.loads(local.read_text(encoding="utf-8"))
    assert saved["youtube_video_id"] == "aBcDeFgHiJk"
    assert saved["youtube_chat_enabled"] is True
    assert saved["streamer_name"] == "bohan", "key lain wajib dipertahankan"
    assert src_path.read_bytes() == before, (
        "source bridge HARAM disentuh — inilah akar state live bocor ke commit"
    )
    assert not list(tmp_path.glob("config_local.json.tmp*")), (
        "temp (per-PID, anti dua-bridge) harus di-rename bersih"
    )


def test_save_config_never_rewrites_own_source_in_code():
    src = BRIDGE_FILE.read_text(encoding="utf-8")
    i = src.index("def _save_config_to_file")
    # Sampai batas fungsi (blok __main__ persis di bawahnya) — jendela fixed
    # 2200 char sempat menyerempet string prompt di luar fungsi (false-pass).
    body = src[i:src.index('\nif __name__', i)]
    # Pola KODE, bukan teks: docstring memang menjelaskan sejarahnya.
    assert '"hermes_vtuber_bridge.py"' not in body, (
        "wizard tidak boleh membuka/menulis source-nya sendiri (drift ke git)"
    )
    assert "re.sub(" not in body, "tidak ada lagi tulis-ulang source via regex"
    assert '"config_local.json"' in body
