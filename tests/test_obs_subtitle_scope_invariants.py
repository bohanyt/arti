...
def test_broadcast_subtitle_gated_to_tts_engine_speak_only():
    ...
    # Sejak refactor dual-engine (supertone-tts-upgrade), broadcast pindah ke
    # helper playback TTSEngine yang HANYA dipanggil dari speak(): _play_wav
    # (jalur Supertone) dan _speak_edge_tts (jalur edge). Spirit Req 6.3 tetap:
    # subtitle hanya untuk ucapan hasil sintesis, tidak pernah untuk chat /
    # social-stream / input mic.
    _SPEAK_ENCLOSED_METHODS = {"speak", "_play_wav", "_speak_edge_tts"}

    bad: list[tuple[int, str | None, str | None]] = []
    for lineno, func_name, class_name in call_sites:
        if func_name not in _SPEAK_ENCLOSED_METHODS or class_name != "TTSEngine":
...
def test_tts_is_playing_read_sites_still_gate_listening():
    ...
    for node, parents in _walk_with_parents(_BRIDGE_TREE):
        ...
        if enc_func == "speak" and enc_class == "TTSEngine":
            continue
        # _idle_paused() (idle rework v0.5.x) adalah helper predicate:
        # `return tts_is_playing` — gating truthy-nya dilakukan pemanggil
        # (`if _idle_paused():` di idle worker), jadi read ini tetap gate.
        if enc_func == "_idle_paused":
            continue
        # Accessor `lambda: tts_is_playing` (mis. get_tts_is_playing untuk
        # desktop_audio worker): konsumen yang melakukan gating truthy.
        if any(isinstance(p, ast.Lambda) for p in parents):
            continue
    ...
    # Bounded structural count: minimal 3 (we never LOSE a gate — Req 5.6).
    # Batas atas 5 dari era task obs-subtitle sudah tidak relevan: fitur
    # v0.5–v0.6 (idle rework, curious, vision, PTT attention) menambah gate
    # baru secara sah — gate tambahan = lebih aman, bukan regresi.
    assert len(gate_sites) >= 3, (
        f"Expected >=3 tts_is_playing gate reads outside TTSEngine.speak, "
