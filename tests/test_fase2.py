...
def test_filter_meta_history_talk():
    ...
    narrator = (
        "Arti ini dipanggil oleh Bohan. Pertanyaan atau panggilan sekarang adalah: oke dengar. "
...
def test_incharacter_fallback_viewers():
    ...
    # Nama streamer kini config-driven; fallback bisa sebut viewer ("keinget")
    # atau minta ulang — dua-duanya in-character.
    assert any(k in fb.lower() for k in ("keinget", "ulang"))
...
def test_finalize_session_artifacts_smoke(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(session_transcript, "groq_summarize_session", fake_groq_summarize)
...
def test_incharacter_fallback_nyala():
    ...
    yt = '[Pesan Live Chat dari Viewer kelap-z (YouTube)]: "Arti kamu nyala gk??"'
...
def test_post_process_truncates_not_fallback_on_long_reply():
    ...
    long_ok = (
        "Halo Bohan! Stream malam ini seru banget sih, penontonnya rame dan banyak yang lucu. "
...
def test_post_process_strips_cjk_characters():
    """qwen Groq nyelipin aksara CJK ('tiap kali直播 lah?') — TTS baca kacau."""
    import hermes_vtuber_bridge as b

    out = b.post_process_response("Bohan, nanya gitu tiap kali直播 lah? Aku siap.", "hai")
    assert "直播" not in out
    assert "tiap kali" in out
    # teks latin murni tidak tersentuh
    assert "halo" in b.post_process_response("halo Bohan apa kabar", "hai")


def test_post_process_shortens_viewer_handles():
    """TTS jangan baca 'penontonsetia241' bulat-bulat — concern Bohan 2026-08-02."""
    import hermes_vtuber_bridge as b

    wrap = "[Pesan Live Chat dari Viewer @penontonsetia241 (YouTube)]: halo arti"
    out = b.post_process_response("Halo penontonsetia241, kamu setia banget!", wrap)
    assert "penontonsetia241" not in out
    assert "penonton" in out
    # handle ber-@ dengan angka ekor dari history juga dipendekkan
    out2 = b.post_process_response("Tadi @namayangpanjangnya7169 nanya juga.", wrap)
    assert "7169" not in out2
    # angka murni dalam jawaban TIDAK tersentuh
    out3 = b.post_process_response("Tinggi model aku 159 cm.", wrap)
    assert "159 cm" in out3
