...
def harness(monkeypatch):
    ...
    # Default engine kini "supertone" — tanpa ini speak() akan spawn subprocess
    # Supertone ASLI (venv312) dulu sebelum jatuh ke FakeCommunicate. Fixture
    # ini menguji alur edge_tts, jadi paksa edge_tts.
    ...
    # speak() diakhiri `await asyncio.sleep(0.3)`; dikali ratusan contoh
    # hypothesis itu berarti >1 menit idle murni. No-op-kan (sama seperti
    # test_obs_subtitle_integration_smoke).
    async def _fake_async_sleep(_seconds):
        return None

    monkeypatch.setattr(bridge.asyncio, "sleep", _fake_async_sleep)
...
# speak() menormalkan input via strip_tts_expression_tags (dan return lebih
# awal kalau hasilnya kosong). Generate teks yang sudah dalam bentuk normal —
# strip idempotent — supaya assertion "full_text == text" tetap valid dan
# input whitespace-only tidak menguji jalur early-return di test yang salah.
_NONEMPTY_TEXT = (
    st.text(min_size=1, max_size=80)
    .map(lambda t: bridge.strip_tts_expression_tags(t))
    .filter(lambda t: t != "")
)
...
def test_property_8_tts_is_playing_assignment_ordering():
    ...
    # Sejak refactor dual-engine, pasangan `tts_is_playing = True` + sd.play
    # tinggal di helper playback (_play_wav / _speak_edge_tts) yang dipanggil
    # dari speak() — periksa SEMUA method TTSEngine, invariannya sama.
    method_nodes = []
    ...
    assert method_nodes, "TTSEngine methods not found"
    ...
    def _check_block(stmts):
        ...
        for i, stmt in enumerate(stmts):
            if (isinstance(stmt, ast.Assign)
                ...
                # Spirit Req 5.4: TIDAK ADA await antara gate dan sd.play.
                # Statement sync sederhana (mis. `play_t0 = time.perf_counter()`
                # dari fitur PipelineTimer) boleh menyela — bukan celah mic.
                def _is_sd_play(s):
                    return (isinstance(s, ast.Expr)
                            and isinstance(s.value, ast.Call)
                            and isinstance(s.value.func, ast.Attribute)
                            and isinstance(s.value.func.value, ast.Name)
                            and s.value.func.value.id == "sd"
                            and s.value.func.attr == "play")

                play_idx = None
                for j in range(i + 1, len(stmts)):
                    if _is_sd_play(stmts[j]):
                        play_idx = j
                        break
                assert play_idx is not None, (
                    "sd.play(...) must appear in the same statement block "
                    "after tts_is_playing = True"
                )
                # Tidak ada await dari assignment s/d sd.play (inklusif).
                for k in range(i, play_idx + 1):
                    for sub in ast.walk(stmts[k]):
                        assert not isinstance(sub, ast.Await), (
                            "no await may appear between tts_is_playing = True "
                            "and sd.play(...) (Req 5.4)"
                        )
                found_pairing = True

    # Walk every nested statement list inside every TTSEngine method.
    ...
    assert found_pairing, (
        ...
        "any TTSEngine method (speak/_play_wav/_speak_edge_tts)"
    )
