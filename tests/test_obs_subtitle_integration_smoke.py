...
- Task 8.5: ``python subtitle_server.py`` standalone still binds its default port (9988).
...
STANDALONE_PORT = 9988  # port default subtitle_server.py (dulu 9999, pindah ke 9988)
...
def _measure_speak_latency(*, enabled: bool, server_started: bool) -> float:
    ...
    # Default engine kini "supertone"; test ini mengukur jalur edge_tts yang
    # di-fake (_FakeCommunicate), jadi paksa edge_tts selama pengukuran.
    saved_engine = hermes_vtuber_bridge.CONFIG.get("tts_engine")

    try:
        # Configure the failure-mode / baseline branch.
        hermes_vtuber_bridge.CONFIG["tts_engine"] = "edge_tts"
        ...
        hermes_vtuber_bridge.CONFIG["tts_engine"] = saved_engine
...
# standalone still binds its default port (9988).
# Validates Requirement 3.4.
def test_standalone_subtitle_server_binds_default_port():
    """Spawning ``subtitle_server.py`` as a subprocess must end with a
    process that is listening on its default TCP port (9988), exactly as the unmodified
    ``__main__`` block of the file promises."""

    if _port_is_already_bound("127.0.0.1", STANDALONE_PORT):
        pytest.skip(
            "subtitle port 9988 already in use; cannot run standalone subtitle_server "
    ...
    try:
        ...
        while time.perf_counter() < deadline:
            ...
            if proc.poll() is not None:
                ...
                pytest.fail(
                    "subtitle_server.py exited before binding the default port. "
                    f"returncode={proc.returncode}\n"
                    f"stdout={stdout!r}\nstderr={stderr!r}"
                )
            try:
                with socket.create_connection(
                    ("127.0.0.1", STANDALONE_PORT), timeout=0.5
                ):
                    bound = True
                    break
        ...
        assert bound, (
            "subtitle_server.py did not bind 127.0.0.1:9988 within "
