...
def write_beats_md(beats: list[BeatDraft], path: Path | str, session_id: str) -> Path:
    """Tulis beats terkurasi ke Markdown.

    Segmen `rejected` SENGAJA tidak ditulis ke sini. Berkas ini tertangkap glob
    `vault/**/*.md` di `arti_vault_rag.py` dan ikut di-embed jadi memori Arti — sedangkan
    segmen rejected sering berisi keluaran model mentah, termasuk potongan prompt sistem
    ("We need to output JSON only, with fields: summary..."). Kalau ikut ter-embed, RAG
    bisa mengembalikan instruksi internal sebagai "ingatan".

    Ini menyamakan perilaku dengan `arti_curator.append_timeline_to_session_md` (baris
    104) yang sudah memfilter `!= "approved"`. Jejak audit penuh — termasuk yang rejected
    beserta alasannya — tetap tersimpan di `*_beats.jsonl` lewat `write_beats_jsonl`,
    dan berkas jsonl di vault TIDAK ikut di-index (glob jsonl hanya mencakup
    `transcripts/`).
    """
    p = Path(path)
    p.parent.mkdir(parents=True, exist_ok=True)
    ...
    if n_skipped:
        lines.append(
            f"<!-- {n_skipped} segmen rejected tidak ditulis ke sini "
            f"(sengaja, agar tidak ikut di-embed); rekam lengkapnya ada di *_beats.jsonl -->"
        )
        lines.append("")
