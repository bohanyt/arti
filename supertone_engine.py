...
CUDA_ENV_FLAG = "ARTI_SUPERTONE_CUDA"


def enable_cuda_if_requested() -> str:
    """Arahkan Supertonic ke GPU kalau bridge memintanya. Return status untuk log.

    Kenapa serumit ini: ``supertonic.TTS.__init__`` TIDAK punya argumen device —
    hanya jumlah thread CPU. Providernya diambil dari konstanta modul
    ``supertonic.config.DEFAULT_ONNX_PROVIDERS = ["CPUExecutionProvider"]``, yang di
    sumbernya sendiri diberi komentar "GPU support can be added by extending this list".

    Dua detail yang mudah salah:

    1. Tambal ``supertonic.loader``, BUKAN ``supertonic.config``. loader.py melakukan
       ``from .config import DEFAULT_ONNX_PROVIDERS``, jadi nilainya sudah disalin ke
       namespace loader — mengubah config tidak berpengaruh apa pun.

    2. DLL CUDA harus masuk PATH SEBELUM ``onnxruntime`` di-import. DLL itu datang dari
       paket pip ``nvidia-*`` di dalam venv312. Bridge sudah punya kode serupa
       (hermes_vtuber_bridge.py baris 12-17) TAPI dihitung dari ``sys.executable``-nya
       sendiri — yaitu venv 3.11, bukan venv312. Subprocess ini mewarisi PATH yang
       menunjuk venv yang salah, jadi ia harus menyiapkan miliknya sendiri.

    Aman kalau gagal: ``supertonic/loader.py`` memfilter ke provider yang benar-benar
    tersedia dan jatuh ke CPU dengan warning. Terverifikasi — saat runtime CUDA belum
    terpasang, synth tetap jalan (di CPU) alih-alih meledak.
    """
    if os.environ.get(CUDA_ENV_FLAG, "").strip().lower() not in ("1", "true", "yes"):
        return "cuda: tidak diminta (CPU)"
        ...
        if os.path.isdir(d):
            ...
            if hasattr(os, "add_dll_directory"):
                try:
                    ...
                    pass
            added.append(os.path.basename(os.path.dirname(d)))

    try:
        import supertonic.loader as _loader

        _loader.DEFAULT_ONNX_PROVIDERS = ["CUDAExecutionProvider", "CPUExecutionProvider"]
    except Exception as exc:  # noqa: BLE001 — jangan pernah menggagalkan boot karena ini
        return f"cuda: gagal menambal provider ({type(exc).__name__}) — lanjut CPU"

    return f"cuda: diminta; dll dirs={added or 'tidak ada'}"
...
def load_model() -> None:
    ...
    try:
        ...
        log(f"onnxruntime {_ort.__version__} providers={_ort.get_available_providers()}")
    except Exception:  # noqa: BLE001 — informatif saja
        pass
